...
Concept Suggestor aims to eliminate concepts that are too identical to each other. It is capable of doing this with concepts up to two words large. For this, it uses a pipeline constructed from several smaller algorithms that build upon each other.

The two smallest algorithms \ref{al:synonymity} and \ref{al:semanticsimilarity}  determine synonymity of two words and determine similarity of two words respectively.
...
\begin{algorithm}
\caption{Determining similarity of two words.}\label{al:semanticsimilarity}
\begin{algorithmic}[1]
...
These two algorithms can be used to construct algorithm \ref{al:twowordpipeline}, the pipeline for two single-word concepts.
...
Finally, the pipeline for two single-word concepts can be used for building algorithm \ref{al:onetwocompoundpipeline}, an algorithm that compares a single-word concept to a two-word concept and also for algorithm \ref{al:twocompoundpipeline}, an algorithm that compares a two-word concept to another two-word concept.
